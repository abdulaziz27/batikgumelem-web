    ...
    <section class="bg-gray-100 py-8 px-4">
      <div class="container mx-auto">
        <ul class="breadcrumb flex items-center space-x-2 text-sm">
            ...
            <li><a href="{{ route('products') }}" class="text-grey-700 hover:text-brown-600">{{ __('messages.products') }}</a></li>
        ...
        <div class="w-full md:hidden px-4">
            ...
            @if($product->sizes->sum('stock') > 0)
                <div class="mt-2 text-green-600">
                    {{ __('messages.in_stock') }} ({{ $product->sizes->sum('stock') }} {{ __('messages.items_available') }})
                </div>
            @else
                ...
                <div class="grid grid-cols-2 gap-2 md:grid-cols-3 lg:grid-cols-4">
                    @forelse($product->sizes as $size)
                        <div class="relative">
                            <input type="radio" name="size_id" id="size-{{ $size->id }}" value="{{ $size->id }}"
                                ...
                                {{ $loop->first && $size->stock > 0 ? 'checked' : '' }}
                                required>
                            <label for="size-{{ $size->id }}"
                                ...
                                @if($size->description)
                                    <span class="text-xs text-gray-500 text-center mt-1">{{ $size->description }}</span>
                                @endif
                                <span class="mt-2 {{ $size->stock > 0 ? 'text-green-600' : 'text-red-600' }} text-sm">
                    ...
                    @empty
                        <div class="col-span-full p-4 border rounded-lg bg-yellow-50 text-yellow-800">
                            <p>No size options are available for this product. Please contact customer service for more information.</p>
                        </div>
                    @endforelse
                </div>
                @if($product->sizes->count() > 0)
                    <div class="mt-2">
                        <a href="#size-guide" class="text-brown-600 hover:underline text-sm inline-flex items-center">
          ...
          <div class="flex flex-wrap items-center mt-4">
            ...
            @auth
                ...
                <form action="{{ route('wishlist.toggle', $product->id) }}" method="POST" class="inline-block">
                    @csrf
                    <button type="submit" class="inline-flex items-center px-4 py-3 ml-3 border border-gray-300 rounded-full hover:bg-gray-100 transition duration-200 wishlist-button {{ $inWishlist ? 'text-red-600' : 'text-gray-600' }}">
                        <svg class="w-5 h-5 mr-1" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                            <path fill-rule="evenodd" d="M3.172 5.172a4 4 0 015.656 0L10 6.343l1.172-1.171a4 4 0 115.656 5.656L10 17.657l-6.828-6.829a4 4 0 010-5.656z" clip-rule="evenodd"></path>
                        </svg>
                        {{ $inWishlist ? __('messages.saved') : __('messages.add_to_wishlist') }}
                    </button>
                </form>
            @else
                <a href="{{ route('login') }}" class="inline-flex items-center px-4 py-3 ml-3 border border-gray-300 rounded-full hover:bg-gray-100 transition duration-200 text-gray-600">
                    <svg class="w-5 h-5 mr-1" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                        <path fill-rule="evenodd" d="M3.172 5.172a4 4 0 015.656 0L10 6.343l1.172-1.171a4 4 0 115.656 5.656L10 17.657l-6.828-6.829a4 4 0 010-5.656z" clip-rule="evenodd"></path>
                    </svg>
